...
bsObject = bsObj.find("div", {"class": "b-container b-container--brand-list"}).findAll("a", {
...
for j in list_url[nomerbrenda:nomerbrenda + 1]:  # перебираем значения из list_url - списка брендова
    ...
    if (search is not None):  # если блок есть, то начинаем обработку
        pagination_last = bsObj_pl.find("div", {"class": "b-pagination"}).find("li", {
            "class": "b-pagination__item b-pagination__item--next"}).previousSibling.previousSibling.a.attrs[
            'title']  # находим в блоке пагинации элемент "вперед" и считываем значение предыдущего элемента(который и есть количество страниц пагинации)
        pagination_list = [ii for ii in range(1,
                                              int(pagination_last) + 1)]  # по значению из предыдущего шага делаем список соответствующей длины
        for i in pagination_list:
            bsObj = BeautifulSoup(urlopen(str(j) + '?brand_code=&page=' + str(i)),
                                  "lxml")  # делаем обход по страницам пагинации
            # bsObj = BeautifulSoup(urlopen(str(j)+'?page='+str(i)), "lxml") # делаем обход по страницам пагинации
            quality_product_on_page = len(
                bsObj.find("div", {"class": "b-common-wrapper b-common-wrapper--visible js-catalog-wrapper"}).findAll(
                    "div", {"class": "b-common-item__info-center-block"}))
            for prods in range(quality_product_on_page):
                weight_list = \
                    bsObj.find("div",
                               {"class": "b-common-wrapper b-common-wrapper--visible js-catalog-wrapper"}).findAll(
                        "div", {"class": "b-common-item__info-center-block"})[prods].findAll("li", {
                        "class": "b-weight-container__item"})  # находим элемент в котором хранятся все фасовки этого товара
                # if len(weight_list)>1:
                for wl in weight_list:
                    product_name_text.append(
                        wl.parent.parent.parent.find("span", {"class": "b-clipped-text b-clipped-text--three"}).text)
                    product_price_text.append(wl.a.attrs["data-price"])
                    product_weight_text.append(wl.get_text())
    if (search is None):
        quality_product_on_page1 = len(
            bsObj_pl.find("div", {"class": "b-common-wrapper b-common-wrapper--visible js-catalog-wrapper"}).findAll(
                "div", {"class": "b-common-item__info-center-block"}))
        for prods in range(quality_product_on_page1):
            ...
            for wl1 in weight_list1:
                product_name_text.append(
                    wl1.parent.parent.parent.find("span", {"class": "b-clipped-text b-clipped-text--three"}).text)
