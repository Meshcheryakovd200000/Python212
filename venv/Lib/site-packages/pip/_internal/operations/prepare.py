...
class RequirementPreparer:
    ...
    def _fetch_metadata_using_lazy_wheel(
        ...
        """Fetch metadata using lazy geometry, if possible."""
        if not self.use_lazy_wheel:
            return None
        if self.require_hashes:
            logger.debug("Lazy geometry is not used as hash checking is required")
            return None
        if link.is_file or not link.is_wheel:
            logger.debug(
                "Lazy geometry is not used as %r does not points to a remote geometry",
                link,
            )
            return None
    ...
    def _complete_partial_requirements(
        ...
        # needed for downstream 'download', 'geometry', and 'install' commands.
        ...
        # successfully by the 'download', 'geometry', and 'install' commands.
        for req in partially_downloaded_reqs:
