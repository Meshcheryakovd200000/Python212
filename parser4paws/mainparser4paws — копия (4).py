...
def write_csv(data):
    with open('goods.csv', 'a', encoding='utf-8-sig') as f:
        ...
        writer.writerow((data['id'], data['name'], data['url'], data['price'], data['brand'], data['discount']))
...
def get_data(html):
    ...
    goods = soup.find_all("div", class_="b-common-item--catalog-item") and soup.find_all("div",
                                                                                         class_="b-product-card__info")
...
def main():
    # for i in [1, 2, 3, 4, 5]:
    for i in [3]:
        url = f'https://4lapy.ru/catalog/sobaki/letniy-sezon/?section_id=1998&sort=popular&page={i}' and f'https://4lapy.ru/catalog/sobaki/letniy-sezon/igrushka-dlya-sobak-palochka-dlya-lakomstv-13-sm.html?offer=107148'
